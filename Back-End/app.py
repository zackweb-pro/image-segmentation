...
if os.path.exists(ENCODER_PATH) and os.path.exists(DECODER_PATH):
    print(
        f"Loading wall segmentation model from {ENCODER_PATH} and {DECODER_PATH}")
    segmentation_model = load_wall_segmentation_model(
    ...
    print(
        f"WARNING: Model weights not found at {ENCODER_PATH} or {DECODER_PATH}")
...
def segment():
    ...
    if width > max_dim or height > max_dim:
        ...
        # Resize the image
        print(
            f"Resizing image from {width}x{height} to {new_width}x{new_height} for faster processing")
